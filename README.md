...
Requires an Apple Silicon Mac, Xcode with the Metal Toolchain, and Python 3.10 or newer.
...
`bench.py` prints median time per call after warmup, with MLX and hardware details. A speedup is not assumed: the result depends on shape, dtype, and Apple Silicon generation.
